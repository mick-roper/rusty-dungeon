...
impl Renderer<'static> {
    pub fn new(width: u32, height: u32) -> Result<Renderer<'static>, String> {
        ...
        Ok(Renderer{context, canvas, texture_creator, texture_manager})
    }
    ...
    pub fn draw(&mut self, state: &mut State) {
        ...
        for (pos, drawable) in (&positions, &drawables).join() {
            ...
            let t = self.texture_manager.load(&drawable.texture_id).unwrap();
